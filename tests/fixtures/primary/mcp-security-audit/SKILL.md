...
description: |
  Audit MCP (Model Context Protocol) server configurations for security issues including
  secrets exposure, shell injection, unpinned dependencies, and dangerous commands.
  Use this skill to review `.mcp.json` files, detect hardcoded secrets, and ensure MCP
  servers follow security best practices.
keywords: [mcp, security, audit, secrets, shell-injection, supply-chain, governance]
categories: [security, audit, mcp]
examples:
  - "Audit .mcp.json for hardcoded secrets"
  - "Check MCP servers for unpinned dependencies"
---
...
.mcp.json → Parse Servers → Check Each Server:
  ...
  5. Is Server in approved_mcp.json in root, If approved_mcp.json is missing or cannot be read, report a CRITICAL finding and fail the audit with a clear error message.
→ Generate Report
```
