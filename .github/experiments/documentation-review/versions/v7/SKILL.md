...
- **Factual Fix** — a Modification that corrects a Factual Statement using Repository Evidence. A Factual Fix must not change the Statement's **meaning** (the set of facts it asserts), **intent** (the action the reader is expected to take as a result of the Statement), or **scope** (the set of subjects, files, commands, or conditions to which the Statement applies). A Factual Fix changes only the words used to express a fact that is already supported by Repository Evidence; it does not add, remove, broaden, or narrow the underlying claim.
