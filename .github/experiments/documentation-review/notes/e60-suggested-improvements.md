...
4. **Add dedup for contradiction-related findings**
...
5. **Test the v2 coverage prompt on production corpus**
...
6. **Try Claude 3.5 Sonnet on the clean test fixtures**
...
7. **Run the multi-model corpus scan with deepseek for ALL waves (not just deep)**
...
8. **Add coverage-gap rule for "What if multiple X are missing" pattern**
...
9. **Improve the test-ambiguities-hard fixture**
...
10. **Add a "code-only" mode that detects undefined variables, unused imports, etc.**
...
11. **Test the v2 coverage prompt on individual fixtures with more runs**
    - E50 only ran 3 runs. Test with 10+ runs to get more stable estimates.

12. **Explore whether a single good model is better than multi-model**
...
13. **Build a "skill-quality-score" that combines all findings into a single grade**
