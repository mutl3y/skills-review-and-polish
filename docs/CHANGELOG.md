...
- **Loop-detection false positives on unchanged documents (analysis-review loop, iter-7).** Re-analyzing byte-identical text fired `llm-loop-detected` because `detectLoops` compared findings against history without consulting the stored document fingerprint. It now returns early when `history.lastFingerprint === currentFingerprint` — an unchanged document reproducing identical findings is a re-review of stable code, not a loop; a loop is when the document *changed* but the same findings recur.
- **Extension agent LM tools ignored the per-session token budget (iter-3).** The `analyze`/`fix` LM tools passed `undefined` to the session-budget cap, silently bypassing the user's `mcpMaxTokensPerSession` setting while the MCP door obeyed it. Both tools now pass `cfg.mcpMaxTokensPerSession`.
- **Extension fix tool missing the duplicate-anchor guard (iter-4).** The extension's `fix` LM tool called `fixer.fixIssue` directly and silently fixed the first occurrence of a duplicated `relevantText`, while the MCP `handleFix` refused. Consolidated into a shared `validateFixAnchor` helper used by both doors.
- **Accepted-findings root divergence (iter-2).** `resolveAcceptedFindingsPath` used `MCP_SERVER_WORKSPACE || cwd` while path safety honored the pinned `workspaceRoot` — when config pins a non-cwd root, store keys and store lookup diverged. Now routed through the single `resolveWorkspaceRoot()`.
- **Wave-count budget divergence (iter-1).** The number of LLM waves an analyze/fix runs was duplicated between the MCP server and the extension and had drifted (extension hardcoded 6; MCP used `estimateWaveCount`). Consolidated into a shared `src/core/waveCount.ts` (`estimateWaveCount`, `estimateFixWaveCount`) imported by both doors.
- **`process.cwd()` fallback (iter-1)** replaced with fail-closed path resolution; **bare URL handling (iter-1)** tightened.
