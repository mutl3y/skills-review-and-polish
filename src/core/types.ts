...
// ─── OpenRouter Batch API types ──────────────────────────────────────────────
// Reference: https://openrouter.ai/docs/batch-quickstart
// Batch-only models return 404 on /api/v1/chat/completions with the message
// "This model is only available through the Batch API. Use the /api/beta/batches
// endpoint instead." — so batch-capable models must be routed to /api/beta/batches.

/** A single request inside a batch submission. */
export interface BatchRequestItem {
  /** Client-supplied id, echoed back in results so items can be correlated. */
  custom_id: string;
  body: {
    model: string;
...
/** Payload sent to POST /api/beta/batches. */
export interface BatchSubmission {
  /** Array of request objects (OpenRouter wraps each in { custom_id, body }). */
  requests: BatchRequestItem[];
}

/** Status of a batch job as returned by GET /api/beta/batches/{id}. */
export type BatchStatus = 'pending' | 'processing' | 'completed' | 'failed' | 'cancelled' | 'expired';

/** A single result item returned when a batch completes. */
export interface BatchResultItem {
  custom_id: string;
  /** Present when this item succeeded. */
  body?: {
    choices?: Array<{ message?: { content?: string }; finish_reason?: string }>;
    error?: { message?: string; code?: number | string };
  };
  /** Present when this individual item failed (e.g. model not batch-capable). */
  error?: { message?: string; code?: number | string } | string;
}

/** Response from GET /api/beta/batches/{id} once completed. */
...
/** Capability flag resolved from the OpenRouter model catalog. */
export interface BatchCapableModel {
  id: string;
